"""
Edits: (primarily on padding)
1. Changes the padding for some places for compatible with (1)linear operation (2)size inp = size out
    (since previously we never encounter the case of even kernel size)
2. Slightly change the behavior when encontering kernel size = (1,k)
    (since previously we didn't consider such cases)
"""
...
# For new version of pytorch (tested on 1.11):
